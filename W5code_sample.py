...
def mk_df(date_info, aud_usd_info, eur_aud_info):
    ...
    merged = pd.merge(merged, eur_aud_info_df, how="outer", on=["row_id"])
    result = pd.DataFrame(merged, columns=['AUD/USD', 'EUR/AUD'])

    date_info_df['date'] = pd.to_datetime(date_info_df['date'])
    date_info_df = date_info_df.sort_values(by='date').reset_index(drop=True)
    return result
    pass
