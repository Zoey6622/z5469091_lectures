...
# object): 这将打开位于`SRCFILE`的文件并返回一个处理程序（文件 对象）：
fobj  = open(SRCFILE, mode = 'r')

# We can get the entire content of the file by calling the method `.read()`,
# without parameters: 我们可以通过调用方法`.read()`来获得整个文件的内容、 来获取整个文件的内容，不需要参数：
cnts  = fobj.read()

# The variable `cnts` will be a string containing the full contents of the
# file. This will print the first 20 characters:  变量`cnts`将是一个包含文件全部内容的字符串。这将打印前20个字符：
print(cnts[:20])

# Check if the file is closed      -- Answer is False means file is open
print(fobj.closed)

# Close the file                   -- 我们将文件关闭
fobj.close()
print(fobj.closed)
...
# ---------------------------------------------------------------------------- 
# Remember that we previously closed the file so we need to open it again 记住，我们之前关闭了文件，所以我们需要再次打开它
fobj = open(SRCFILE, mode='r')
# Contents using `.read`
cnts = fobj.read()
...
# ---------------------------------------------------------------------------- 
#   Writing content to a file / Content manager
# ---------------------------------------------------------------------------- 
...
#   must add it yourself: 请注意，写法没有添加换行符（\n）。你 你必须自己添加它：
# ---------------------------------------------------------------------------- 
