...
series_stk = pd.Series(data=[i for i in prc_date], index=pd.to_datetime([prc_date[i] for i in prc_date]))
...
series_ones = pd.Series(data=[dic[i]-i for i in dic], index=dic)
